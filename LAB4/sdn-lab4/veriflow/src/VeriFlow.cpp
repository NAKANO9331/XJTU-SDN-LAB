...
bool VeriFlow::traverseForwardingGraph(const EquivalenceClass& packetClass, ForwardingGraph* graph, const string& currentLocation, const string& lastHop, unordered_set<string> visited, FILE* fp)
{
    ...
    if (itr->isGateway == true) {
        fprintf(fp, "\n[VeriFlow::traverseForwardingGraph] Packet class reached destination at node %s.\n", currentLocation.c_str());
        fprintf(fp, "[VeriFlow::traverseForwardingGraph] PacketClass: %s\n", packetClass.toString().c_str());
        updateFaults(packetClass, fp);
        return true;
    }
...
void VeriFlow::updateFaults(const EquivalenceClass& packetClass, FILE* fp) {
        //std::cout << "--- Starting updateFaults ---" << std::endl;
        //std::cout << "Before update, faults size: " << faults.size() << std::endl;
        int index = 0;
        for (auto it = faults.begin(); it != faults.end(); ) {
            //std::cout << "\nProcessing fault at index " << index << ":" << std::endl;
            //std::cout << "Fault before update: ";
            //it->printInfo("");

            EquivalenceClass intersection = it->intersection(packetClass);
            if (!intersection.isEmpty()) {
                ...
                *it = diff;
                ...
                if (it->isEmpty()) {
                    //std::cout << "Fault is empty, removing it from faults list." << std::endl;
                    it = faults.erase(it);
                } else {
                    //std::cout << "Fault is not empty, keeping it in faults list." << std::endl;
                    ++it;
                }
            } else {
                //std::cout << "No intersection, keeping the fault in faults list." << std::endl;
                ++it;
            }
            ++index;
        }
        //std::cout << "After update, faults size: " << faults.size() << std::endl;
        //std::cout << "--- Ending updateFaults ---" << std::endl;
    }

bool VeriFlow::isPacketClassInFaults(const EquivalenceClass& packetClass)
{
