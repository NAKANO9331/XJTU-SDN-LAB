...
class NetworkAwareness(app_manager.OSKenApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.weight = 'delay'  # use "hopNumber between 2 nodes" as weight in ShortPath Choosing
        self.switches = None
    ...
    def port_status_handler(self, ev):
        ...
        # log
        self.send_event_to_observers(ofp_event.EventOFPPortStateChange(datapath, msg.reason, msg.desc.port_no),
                                     datapath.state)
    ...
    def send_echo_request(self, switch):
        datapath = switch.dp
        parser = datapath.ofproto_parser
        echo_req = parser.OFPEchoRequest(datapath, data=bytes("%.12f" % time.time(), 'utf-8'))
    ...
    def _get_topology(self):
        ...
        while True:
            ...
            _hosts, _switches, _links = [str(x) for x in hosts], [str(x) for x in switches], [
                str(x) for x in links]  # get new topo elements
